...
public static class CoreCommandsAsync //: ICoreCommandsAsync
{
    ...
    {
        return await BZMPopAsync(db, timeout, [key], minMaxModifier, count);
    }
    ...
    {
        return await BZPopMinAsync(db, [key], timeout);
    }
    ...
    {
        return await BZPopMaxAsync(db, [key], timeout);
    }
    ...
    {
        return await BLMPopAsync(db, timeout, [key], listSide, count);
    }
    ...
    {
        return await BLPopAsync(db, [key], timeout);
    }
    ...
    {
        return await BRPopAsync(db, [key], timeout);
    }
    ...
    {
        var result = await XReadAsync(db, [key], [position], count, timeoutMilliseconds);
    ...
    {
        var result = await XReadGroupAsync(db, groupName, consumerName, [key], [position], count, timeoutMilliseconds, noAck);
        if (result == null || result.Length == 0)
        {
            return null;
        }
